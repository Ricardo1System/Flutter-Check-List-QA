// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'project_model.dart';

// **************************************************************************
// TypeAdapterGenerator
// **************************************************************************

class ProjectModelAdapter extends TypeAdapter<ProjectModel> {
  @override
  final int typeId = 0;

  @override
  ProjectModel read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return ProjectModel(
      name: fields[1] as String,
      description: fields[2] as String,
      modules: (fields[3] as List?)?.cast<ModuleModel>(),
      id: fields[0] as int,
    );
  }

  @override
  void write(BinaryWriter writer, ProjectModel obj) {
    writer
      ..writeByte(4)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.name)
      ..writeByte(2)
      ..write(obj.description)
      ..writeByte(3)
      ..write(obj.modules);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ProjectModelAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}

class ModuleModelAdapter extends TypeAdapter<ModuleModel> {
  @override
  final int typeId = 1;

  @override
  ModuleModel read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return ModuleModel(
      id: fields[0] as int,
      name: fields[1] as String,
      description: fields[2] as String,
      rules: (fields[3] as List?)?.cast<RuleModel>(),
      activities: (fields[4] as List?)?.cast<ActivityModel>(),
      subModules: (fields[5] as List?)?.cast<SubModuleModel>(),
    );
  }

  @override
  void write(BinaryWriter writer, ModuleModel obj) {
    writer
      ..writeByte(6)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.name)
      ..writeByte(2)
      ..write(obj.description)
      ..writeByte(3)
      ..write(obj.rules)
      ..writeByte(4)
      ..write(obj.activities)
      ..writeByte(5)
      ..write(obj.subModules);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ModuleModelAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}

class SubModuleModelAdapter extends TypeAdapter<SubModuleModel> {
  @override
  final int typeId = 2;

  @override
  SubModuleModel read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return SubModuleModel(
      id: fields[0] as int,
      name: fields[1] as String,
      description: fields[2] as String,
      rules: (fields[3] as List?)?.cast<RuleModel>(),
      activities: (fields[4] as List?)?.cast<ActivityModel>(),
    );
  }

  @override
  void write(BinaryWriter writer, SubModuleModel obj) {
    writer
      ..writeByte(5)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.name)
      ..writeByte(2)
      ..write(obj.description)
      ..writeByte(3)
      ..write(obj.rules)
      ..writeByte(4)
      ..write(obj.activities);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is SubModuleModelAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}

class RuleModelAdapter extends TypeAdapter<RuleModel> {
  @override
  final int typeId = 3;

  @override
  RuleModel read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return RuleModel(
      id: fields[0] as int,
      name: fields[1] as String,
      detail: fields[2] as String,
      subModuleId: fields[3] as int?,
    );
  }

  @override
  void write(BinaryWriter writer, RuleModel obj) {
    writer
      ..writeByte(4)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.name)
      ..writeByte(2)
      ..write(obj.detail)
      ..writeByte(3)
      ..write(obj.subModuleId);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is RuleModelAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}

class ActivityModelAdapter extends TypeAdapter<ActivityModel> {
  @override
  final int typeId = 4;

  @override
  ActivityModel read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return ActivityModel(
      id: fields[0] as int,
      name: fields[1] as String,
      detail: fields[2] as String,
      rules: (fields[3] as List?)?.cast<RuleModel>(),
      subModuleId: fields[4] as int?,
    );
  }

  @override
  void write(BinaryWriter writer, ActivityModel obj) {
    writer
      ..writeByte(5)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.name)
      ..writeByte(2)
      ..write(obj.detail)
      ..writeByte(3)
      ..write(obj.rules)
      ..writeByte(4)
      ..write(obj.subModuleId);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ActivityModelAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}
